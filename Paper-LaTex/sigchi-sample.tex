...
\begin{itemize}
  ...
  \item There is signifiant mismatch between participants' preferred input methods and those supported by the current smart glasses. For example, no one used voice, even for discrete commands, and no one used touch-input on the smart glasses -- the two primary interaction methods for Google Glass. In addition, current cameras can only detect in-air gestures in front of users' faces, missing 63\% of the gestures performed by the participants.
\end{itemize}
