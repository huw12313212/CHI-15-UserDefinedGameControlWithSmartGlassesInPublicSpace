...
\emph{Form} of in-air gesture and on-body input are both dominated with hand related input, and we found that the form of on-body input are more complicate than in-air gesture. But the binding of on-body input is more consistent, almost 75\% controls are independent with screen. And we are surprisingly to see that no users design a \emph{direct-binding} or \emph{physical} control with on-body input. The portion of control's flow are similar in this two interaction methods.
 ...
 \begin{figure}[!h]
  ...
  \end{figure} 
   ...
   With in-air gestures(see Figure.\ref{fig:InAirSet}), Although we ask users to perform game control not limited with hand, the result shows that user still prefer using hand gestures over than voice controls, eye gesture and head tilting. And user would use direct-control if they have to do precise task, like selecting an object from many or moving an object to the specific position. Other tasks which are not so precise like select a single option from 4 or make a avatar jump. User will prefer using a in-direct control, for example, tapping 4 different area in-front of their chest or micro hand lift.

  All the controls in our final on-body input set are finger related (see Figure \ref{fig:OnBodyInputSet}). Most of them are using single finger tip to perform gestures on different surfaces. And hand palm seems like the most preferable surface to do these on-body gestures. These controls seems like an trackpad or a proxy touch-screen. The remain controls are finger interaction with single hand. Specificly, using thumb to interact with index finger or ring.

   \subsubsection{Taxonometric Breakdown of User-Defined Game Controls}
   As we should expect, the taxonometric breakdown of the final user-defined control sets(Figure \ref{fig:InAirSet} and \ref{fig:OnBodyInputSet}) are similar to the proportions of all control actions proposed(Figure \ref{fig:InAirTaxonomy} and \ref{fig:OnbodyTaxonomy}). Across all taxonomy categories, the average difference between these two sets were only 3.09\% and 6.31\%.


  \subsection{Mental Model Observations}
    \subsubsection{Social Acceptance and Control Area}
    To our surpirse, about 63\% of in-air ``hand'' gestures from user-defined were not performed in front of their face (See Figure.\ref{fig:figureInAirPorpotion}.E). This behavior conflict with current ``Google Glass'' design and previous works\cite{Colaco:2013:MCL:2501988.2502042}. There are 7 participants perfoming more gesture in front of face than other position, they indicate that control in front of face is more precise and intuitive. At same time, the other 17 users more like to perform in-air gesture in front of their chest or below. There are 3 participants never perform any in-air gesture in front of their face. These users indicate that moving a finger in front of their face is really weird and not social acceptable. And there is also a hand strain problem to lift hand in front of face. They think it is not suitable for game control.



    \subsubsection{Metaphor from Exisiting Game Control}
    Although we took care not to show elements from traditional game platforms like Consoles, PCs or Mobile games, participants still often thought with the previous gaming experience. For example, some controls actions performed as if using touch-screen in front of their face (),


    some control actions performed as if using a imaginary trackpad on in-air surface or hand surface 
  \begin{figure*}
  \centering
